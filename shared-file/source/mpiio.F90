...
subroutine mpiioread(filename, iodata, n1, n2, n3, cartcomm)
  ...
  !TODO: This is all boilerplate code copied from mpiiowrite. Move to a shared routine?
  integer, parameter :: ndim = 3
...
! Serial read/write is unconditionally compiled
...
subroutine serialread(filename, iodata, n1, n2, n3, cartcomm)
...
!  Constantly overwriting array with data from file
  ...
  if (rank == 0) then
     ...
     do i = 1, size
        read(unit=iounit) iodata(1:n1, 1:n2, 1:n3)
     end do
